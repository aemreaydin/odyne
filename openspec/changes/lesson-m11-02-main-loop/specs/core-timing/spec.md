<!-- Engine capability delta for lesson-m11-02-main-loop.
     `core-timing` is a NEW capability: m11-01 built the frame clock as an isolated kata and
     deliberately shipped no spec delta, deferring it to this lesson's graduation so that
     openspec/specs/ only ever claims what engine/ provably does. This delta covers both the
     graduated clock and the new fixed-timestep pacer. -->
