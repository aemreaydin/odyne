<!-- Engine capability delta for lesson-m11-02-main-loop.
     `game-loop` is a NEW capability: the engine-owned frame loop that drives the application
     through callbacks, at the top engine layer (package `game`), so that pumping the platform
     and calling the application are both downward calls and the layering law is untouched. -->
